...
function installFile(f, pakcfg, paths, dep, silent, options, callback) {
    ...
    var logger = function (src, dst, error) {
        if (!error) {
            if (!silent) {
                console.log(('\t' + key + ' : ' + dst).green);
            }
        } else {
            if (!silent) {
                console.log(('Error\t' + src + ' : ' + dst).red);
                console.log('\t\t' + error);
            }
        }
    };
    ...
    if (f_name !== f_path) {
        if (fs.lstatSync(f_name).isFile()) {
            utils.copyFile(f_name, f_path, function (error) {
                logger(f, f_path, error);

                callback(error);
            });
        } else {
            utils.copyDir(f_name, f_path, logger, callback);
        }
    } else {
        callback();
    }
